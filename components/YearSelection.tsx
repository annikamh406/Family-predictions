...
type GameYearRow = { year: number; status: string }
...
export function YearSelection({ onSelectYear }: { onSelectYear: (year: number) => void }) {
    ...
    const handleCreateYear = async () => {
        ...
        const { error } = await supabase
            .from('game_years')
            .insert([{ year: yearInt, status: 'forecasting' }])
    ...
    const handleDeleteYear = async (e: React.MouseEvent, yearToDelete: number) => {
        e.stopPropagation() // Prevent entering the year when clicking delete
        ...
        <div className="max-w-4xl mx-auto space-y-12 py-12 animate-in fade-in zoom-in duration-500">
            <div className="text-center space-y-4">
                ...
                <p className="text-stone-500 max-w-md mx-auto">
                    Travel back in time to see past results or jump into the current game.
                </p>
            </div>
